                ...
                <div class="hero-carousel__viewport">
                    <div class="hero-carousel__track" id="heroSlides_${index}">
                        ${sliders.map((slider, idx) => {
                            let imageUrl = slider.imageUpload?.url || slider.image || getGlobalFallbackImage();
                            let mobileImageUrl = slider.imageMobileUpload?.url || slider.imageMobile || imageUrl;
                            
                            // Optimize image URLs for mobile/desktop
                            if (typeof window.getOptimizedImageUrl === 'function') {
                                imageUrl = window.getOptimizedImageUrl(imageUrl, {
                                    width: 1200,
                                    quality: 'auto:good',
                                    crop: 'limit'
                                });
                                mobileImageUrl = window.getOptimizedImageUrl(mobileImageUrl, {
                                    width: 800,
                                    quality: 'auto:low',
                                    crop: 'limit'
                                });
                            ...
                            const originalImageUrl = slider.imageUpload?.url || slider.image || getGlobalFallbackImage();
                            let srcsetAttr = '';
                            let sizesAttr = '';
                            
                            if (typeof window.generateSrcset === 'function' && originalImageUrl.includes('res.cloudinary.com')) {
                                srcsetAttr = `srcset="${window.generateSrcset(originalImageUrl)}"`;
                                ...
                                if (youtubeId) {
                                    ...
                                    mediaContent = `
                                        <picture>
                                            ...
                                            <img src="${htmlEscape(imageUrl)}" ${srcsetAttr} ${sizesAttr} alt="${htmlEscape(slider.imageAlt || slider.title)}" loading="${idx === 0 ? 'eager' : 'lazy'}">
                                        </picture>
                                    `;
                                }
...
function renderBannerHTML(banner, section) {
    let imageUrl = banner.imageUpload?.url || banner.image || getGlobalFallbackImage();
    
    // Optimize image URL for mobile/desktop
    if (typeof window.getOptimizedImageUrl === 'function') {
    ...
    // Generate srcset for responsive images
    const originalImageUrl = banner.imageUpload?.url || banner.image || getGlobalFallbackImage();
    let srcsetAttr = '';
    let sizesAttr = '';
    ...
    if (isYouTube) {
        ...
        if (youtubeId) {
            ...
            // Fallback to image if YouTube ID extraction fails
            mediaContent = `<img src="${htmlEscape(imageUrl)}" ${srcsetAttr} ${sizesAttr} alt="${htmlEscape(banner.imageAlt || banner.title || 'Banner')}" class="banner-full-width__image" loading="lazy">`;
        }
    } else if (isVimeo) {
...
function renderProductCard(product) {
    let imageUrl = product.imageUpload?.url || product.image || getGlobalFallbackImage();
    ...
    // Generate srcset for responsive images
    const originalImageUrl = product.imageUpload?.url || product.image || getGlobalFallbackImage();
    let srcsetAttr = '';
    let sizesAttr = '';
    
    if (typeof window.generateSrcset === 'function' && originalImageUrl.includes('res.cloudinary.com')) {
        srcsetAttr = `srcset="${window.generateSrcset(originalImageUrl)}"`;
        sizesAttr = 'sizes="(max-width: 480px) 100vw, (max-width: 768px) 50vw, 25vw"';
    }
